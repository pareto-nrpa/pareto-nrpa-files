...
- **City coordinates**: Stored in `coordinates_[instance].pickle`, containing the 2D positions of cities for all instances.
- **Cost matrices**: Stored as `[instance].npy` files, each encoding the pairwise Euclidean distance matrix for a corresponding instance.
